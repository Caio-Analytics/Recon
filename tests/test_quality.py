...
def _stats(**overrides):
    base = {
        ...
        "flags": {
            ...
            "detected_pattern": "Nenhum",
            "stats_suprimidas_lgpd": False,
        },
        "qualidade": {},
        "otimizacao": {},
...
def test_ordenacao_por_prioridade_usa_rank_explicito():
    ...
    assert [r["Prioridade"] for r in ordenadas] == [
        quality.PRIORIDADE_ALTA,
        quality.PRIORIDADE_MEDIA,
        quality.PRIORIDADE_BAIXA,
    ]
    ...
    stats = _stats(
        qualidade={
            "inconsistencia_normalizacao": {
                ...
                "valores_unicos_atual": 5,
                "valores_unicos_normalizado": 2,
                "grupos_afetados": 1,
...
def test_pii_em_texto_livre_gera_recomendacao_lgpd():
    ...
    assert any(
        "LGPD" in r["Acao"] and r["Prioridade"] == quality.PRIORIDADE_ALTA for r in recomendacoes
    )
...
def test_duplicatas_derrubam_o_score():
    ...
    com_dup = quality.calcular_score_qualidade(colunas, {"pct_linhas_duplicadas": 0.5}, [])["score"]
