...
**Profiler exploratório de dados para CSV, XLSX, XLS e XLSB** — infere semântica de colunas, roda testes estatísticos, detecta dependências funcionais e dados sensíveis (LGPD), e gera recomendações de ETL prontas pra usar. Saída simultânea em dois formatos: **JSON** (pra processamento externo/código) e **Markdown** (pra leitura humana).
...
│   ├── reporting.py          exporta JSON (processamento externo), Markdown (humano) e Parquet (opcional)
