...
Roda em CSV, XLSX, XLS e XLSB. Sem banco de dados, sem serviço externo, sem modelo de processamento externo — só Python e as bibliotecas do `pyproject.toml`, instaláveis com `pip install --user` em máquina corporativa. Saída em **JSON** (pra colar num prompt ou consumir via código), **Markdown** e **HTML** (pra ler) e **Parquet** (pra BI).
...
- `profiler_output_arquivo.json` — estrutura completa, pra colar num prompt de processamento externo ou consumir via código
...
**Fase 6** (planejada):
- gráficos embutidos no HTML (histograma e série temporal por coluna)
- calibrar os pesos de confiança da cascata semântica sobre um corpus rotulado
- gazetteer de municípios brasileiros completo (hoje só as 27 capitais)
