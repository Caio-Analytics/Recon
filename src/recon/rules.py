...
def _regra_de_faixa(
    ...
    return {
        "tipo": f"Regra de {pacote}",
        "pacote": pacote,
        "regra": f"`{coluna}` entre {minimo:g} e {maximo:g}",
        "descricao": descricao
        if violacoes.empty
        else f"{descricao} Há {len(violacoes)} valor(es) fora da faixa.",
...
def detectar_regras_por_pacote(
    ...
    if descontos and brutos:
        ...
        if len(pares) >= MIN_LINHAS_REGRA:
            ...
            if taxa >= CONFORMIDADE_MINIMA:
                ...
                achados.append(
                    {
                        "tipo": "Regra de Financeiro",
                        "pacote": "Financeiro",
                        "regra": f"`{desconto}` entre 0 e `{bruto}`",
                        "descricao": (
                            "O desconto é compatível com o valor bruto."
                            if erros.empty
                            else f"{len(erros)} linha(s) têm desconto negativo ou acima do bruto."
                        ),
                        "conformidade": round(taxa, 4),
                        "qtd_violacoes": int(len(erros)),
    ...
    fins = [
        nome
        for nome, tokens in nomes.items()
        if {"resolucao", "conclusao", "fechamento", "encerramento"} & tokens
    ]
    if prazos and fins:
        ...
        if len(pares) >= MIN_LINHAS_REGRA:
            ...
            if taxa >= CONFORMIDADE_MINIMA:
                ...
                achados.append(
                    {
                        "tipo": "Regra de Suporte",
                        "pacote": "Suporte / SLA",
                        "regra": f"`{fim}` <= `{prazo}`",
                        "descricao": "Os chamados foram concluídos até o prazo de SLA."
                        if erros.empty
                        else (f"{len(erros)} chamado(s) foram concluídos após o prazo de SLA."),
                        "conformidade": round(taxa, 4),
                        "qtd_violacoes": int(len(erros)),
                        "exemplos_violacao": erros.head(_MAX_EXEMPLOS)
                        .astype(str)
                        .to_dict("records"),
                    }
                )
...
def _colunas_por_tipo(colunas_meta: list[dict[str, Any]], df: pd.DataFrame):
    ...
    return (
        datas[:MAX_COLUNAS_DATA],
        numericas[:MAX_COLUNAS_NUMERICAS],
            ...
            achados.append(
                {
                    ...
                    "descricao": (
                        ...
                        if violacoes.empty
                        else f"'{antes}' é posterior a '{depois}' em {len(violacoes)} linha(s)"
                    ),
                    "conformidade": round(taxa, 4),
                    "qtd_violacoes": int(len(violacoes)),
                    "exemplos_violacao": [
                            ...
                            taxa,
                            {
                                ...
                                "descricao": (
                                    ...
                                    + (
                                        ...
                                        if violacoes.empty
                                        else f"em {taxa:.1%} das linhas; {len(violacoes)} não fecham a conta."
                                    )
                                ),
                                "conformidade": round(taxa, 4),
                                "qtd_violacoes": int(len(violacoes)),
...
def inferir_regras(df: pd.DataFrame, colunas_meta: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if len(df) < MIN_LINHAS_REGRA:
        return []
